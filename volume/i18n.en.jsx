/* =====================================================================
   VOLUME — i18n.en.jsx  ·  os espelhos ingleses do conteúdo

   Isto SAIU de i18n.jsx em 31/08, e o motivo é peso: são 78 KB crus, 29 KB
   comprimidos, de texto que só existe para `/en`. Enquanto morava no mesmo
   arquivo, TODA visita em português baixava e o V8 parseava os treze objetos
   deste bloco para jogar fora — no aparelho e na rede que o PageSpeed mede,
   isso é banda no caminho crítico e tarefa longa na thread principal, que é
   30% da nota.

   Continua sendo um script CLÁSSICO, e é isso que faz funcionar: `CHAPTERS`,
   `PROJECTS` e `COMPANIES` são `const` de topo declarados em data.jsx, e
   declaração léxica de topo de script clássico vive no ESCOPO GLOBAL DE
   SCRIPT, que é compartilhado entre <script> diferentes. Por isso o bloco
   alcança os globais do outro arquivo por referência nua, exatamente como
   alcançava quando os dois estavam juntos. Um `type="module"` aqui quebraria
   isso em silêncio.

   `LANG` vem de i18n.jsx pelo mesmo mecanismo. A ORDEM DAS TAGS é o contrato:
   data → i18n → i18n.en. Ver buildHtml() em build.mjs.

   Este arquivo só entra no HTML das páginas em inglês.
   ===================================================================== */
...
if (LANG === "en") {
  ...
  const CH = {
    pcyes: {
      ...
      tldr: { papel: "UX/UI Designer, project owner",
        ...
        resultado: "The direction opposite to the brief is the one that got approved. The urgent fix is already live and measured; the V2 ships in October 2026" },
      ...
      notaSuporte: {
        k: "Before we start",
        p: "This project was designed mobile first, and you will see phone screens along the way. Store traffic splits about evenly between computer and phone, but the people buying on a computer spend more, so the large exhibits in this chapter are desktop screenshots.",
        p2: "The average order is R$ 663 on desktop against R$ 420 on mobile. People who build their own PC compare spec sheets, keep several tabs open and decide on the big screen. The phone, on the other hand, converts better and bounces less — which is why the design started there.",
        fonte: "Google Analytics 4 · Q2 2026 · months without the automated traffic spike",
      ...
      abertura: { k: "The scene", t: "A site the company liked and the customer didn't use",
        p: ["PCYES sells hardware and peripherals to people who build their own PC. They arrive knowing the model they want, compare spec sheets against the competition and decide in minutes. The site they had wasn't built for that: it opened on a brand campaign, with video and animation filling the entire first fold, and the product started below the edge of the screen.",
            "What came to me was an aesthetic brief, something along the lines of make it more minimal, cleaner, closer to a big brand. I agreed to look, but not to design before understanding why a beautiful store was selling less than it could, so I spent the whole first week watching users try to buy."],
        fig: "abertura" },
      ...
      painel: {
        ...
        barras: [
          ...
          { l: "Direct traffic, no declared source", v: 50, n: "Half the volume, with a 71.9% bounce rate and 3% of the revenue: almost none of it was users buying." },
        ],
      ...
      busca: {
        ...
        dado: { v: 71416, p: 43, l: "sessions used search in the quarter",
          n: "More users searched than browsed. Search was the store's main path, and it was the one returning a mousepad to anyone looking for a mouse.",
          fonte: "Google Analytics 4 · view_search_results event · Q2 2026" },
      },
      investigacao: { t: "Session recordings instead of opinion",
        p: ["None of this came out of a report. It came out of Clarity, of navigation metrics and of talking to people who were trying to buy.",
      ...
      recusei: { k: "What I turned down",
        ...
        itens: [
          ...
          { o: "Filing a ticket and waiting", r: "The payment module bug was blocking purchases. Diagnosing it wasn't my job, but it was what unlocked the most expensive step in the funnel." },
        ] },
      sistema: {
      ...
      ponte: { k: "Before the V2 · the V1.2", t: "The checkout got better without waiting for the redesign",
          ...
          passos: [
            ...
            { k: "The fix", t: "All four methods on one row",
              p: "The four now sit side by side. On shipping, the store had just switched on several carriers and was dumping more than ten at once: I cut it to the three best by price and relevance, and the rest went behind a \u201csee more\u201d that starts closed. The same checkout got 60% shorter: 3421 pixels tall against 1366.",
              fig: "ckV12" },
            { k: "The names", t: "\u201cNormal\u201d is not the name of a delivery",
              p: "Every carrier reached the screen called \u201cNormal\u201d, which was how the system sorted them internally. Nobody picks between four \u201cnormals\u201d: they now show up under their real names, Correios PAC, Correios SEDEX and each carrier's actual service. It saves no pixels, and it is the difference between a list that exists and a list you can use.",
              fig: "ckV12" },
          ...
          efeito: {
            ...
            dados: [
              ...
              { v: "51/51", l: "card orders completed", n: "none cancelled in the period" },
              { v: "+91%", l: "median card order value", n: "R$ 425 to R$ 812" },
            ],
            fonte: "GA4 and Microsoft Clarity · 27 Jul to 30 Aug 2026 against 1 Feb to 7 Jun · plus the 792-order Magento export",
            ressalvaK: "What this number does not prove",
            ressalva: "The new checkout shipped between 17 and 24 July and the colour changed on the 29th. Five days apart, and no observational analysis can tell which of the two moved add-to-cart. It went up after both, measured per product viewed rather than per session precisely to keep bots out of the denominator. Separating them for real would take an A/B test with both colours live at once.",
          } },
      modulos: [
      ...
      resultado: { t: "Goes live in October",
        p: ["Buying no longer needs the product page, the price stopped disappearing on scroll, and the payment method shows up before the person decides whether to go on. The final direction was approved and the date is set.",
            "What is live is the V1.2, and it measured: fewer script errors in checkout, more people adding to cart, and card orders completing without a single cancellation. The numbers are in its own block, further up.",
            "For the V2 I have no operational numbers, because it hasn't shipped, and I'd rather not present a number that doesn't exist. What I can say now is what will be measured once it does."],
    ...
    "locarmais-conciliacao": {
      ...
      investigacao: { t: "Next to the people who run it daily",
        p: ["I worked with the finance team and followed the real checking routine, to see where the effort went and where the errors appeared.",
        ...
        achados: ["Most entries settle on their own, and the team's time went into hunting the divergent minority inside the correct majority. Reconciling, in practice, is finding what doesn't match.",
      ...
      solucao: { t: "Reconciling means finding what doesn't match",
        p: ["The module opens on the divergent queue, with the accumulated difference highlighted just above it.",
      ...
      aprendi: { p: ["The decision that paid off most was the smallest one, which was requiring a reason to close an entry with a difference. The exception was the hole in the process, and turning it into a required field with a fixed list made it the information that tells which discrepancies recur and with which acquirer.",
    ...
    odex: {
      ...
      problema: { t: "The interface aged along with the product",
        p: ["The platform worked, and had worked for years. What had aged was the interface, an old layout that ended up promising less than the business already delivers today.",
            "In a system with that much use behind it, touching the path costs dearly. The people who operate it every day know where everything is by heart, and making them relearn it would charge for an improvement they never asked for, so the entire surface changed and the logic stayed exactly where it was."] },
      decisoes: [
        ...
        { d: "Validation with managers and the board", r: "because an opinion from someone who doesn't use the product every day is not a criterion." },
      ],
      ...
      aprendi: { p: ["The temptation in a system with years of use is to reorganize everything at once, and holding that back was half the work.",
    ...
    "oderco-revenda": {
      ...
      problema: { t: "Why should I answer all of this?",
        p: ["Traffic arrived from ads straight onto the signup page, and the page had nothing but the form. That works when the person already knows the company, but outside the region almost nobody does.",
            "So the visitor landed on a long form from a brand they had never heard of, and the question they asked was reasonable. Two problems stacked, each feeding the other: there was no context, and the form was too heavy for the level of trust that existed at that moment."] },
      decisoes: [
      ...
      solucao: { t: "The page answers before the form does",
        p: ["Someone arriving from an ad who has never heard of the company finds, on the same screen as the form, what it distributes, who it sells to and what the reseller gets out of it, so that whoever reaches the form already knows what they are signing up for."] },
      resultado: { t: "One system fewer in sales",
        p: ["Operations ran three systems for the same sales process. Today it runs two.",
            "That wasn't the predicted result. To build the automation I went after the RD Station API and documented how it worked, and that showed the development team the integration was simpler than it looked, so they started connecting RD to the internal CRM. One system is out, and the path now is consolidating everything inside the in-house CRM, now that the flow has been validated in practice with the reps.",
      ...
      aprendi: { p: ["What matters in a form is where you cut. At the right point, a drop-off still leaves a contact that sales can pick up later.",
                     "And most of the work wasn't even in the form: it was in answering, before it, the question the visitor was asking in silence, which is who is this company and why should I trust it."] },
    },
    portfolio: {
      ...
      problema: { t: "Prove UX without saying I do UX",
        p: ["The average portfolio lists screens and job titles. I wanted the navigation itself to be the proof: if I guide you well through here, I have already answered whether I can guide a user.",
            "The constraint I set myself was to use no generic template, because a portfolio that could belong to anyone proves nothing about who made it."] },
      decisoes: [
      ...
      solucao: { t: "Cover, chapters, process and afterword",
        p: ["The volume opens on a cover, lists the chapters, turns the page like manga and uses ink and screentone to separate the sections. Anyone in a hurry gets a short path and anyone who wants the full case gets the long one, and both are stated right at the chapter entrance, with the reading time measured."] },
      ...
      resultado: { t: "Zero violations, measured and not promised",
        p: ["The volume passes WCAG 2.1 AA with no violations: zero on paper, zero in ink mode, zero in English and zero at 390px, swept with axe-core on the served page, view by view. Across six widths, from 1920 down to 390, none scrolls horizontally, and the eight routes load without a single JavaScript error.",
            "It is unglamorous, but a portfolio arguing for care with detail while failing an automated sweep contradicts itself in its own shop window. It was the number I could hold myself to, so it is the one that is here, with the tool and the method up front."] },
    },
  };
  CHAPTERS.forEach((c) => {
    const en = CH[c.id];
  ...
  /* o texto longo do card de projeto (`sobre`) nunca teve traducao e
     renderizava em portugues no modo EN: 14 blocos de prosa na home. */
  const PJ_SOBRE_EN = {
    "ux-balas": "A B2B purchase doesn't end at the card: it ends at an invoice with payment terms, and whoever clicks the button will have to justify the order to finance later. So the checkout is deliberately slow in the right places, with short steps, one decision at a time and a full review before closing. Nobody signs off thousands in the dark.",
    "hub-oderco": "Seven brands under one group are the same job seven times over: write it, build the material, publish, repeat. Before the hub, the answer to where is this product now came by message, and every answer cost the attention of two people. Now the whole production line lives in one place and each item's stage is on screen for anyone who wants to look.",
    "ponto-admin": "Time tracking for hotel day staff has two owners who cannot share a screen. The person clocking in is standing up, outdoors, phone in hand, with ten seconds of patience. The person closing payroll needs filters, exceptions and history. I designed both ends so the record leaves one and arrives at the other without anyone retyping in between, which is where payroll usually bleeds.",
    "kitamo-app": "Nobody loses track of their own debt out of laziness: they lose it because it sits in six places that never show up together. Pulling it into one view and projecting the month moves the question. It stops being how much do I owe, which frightens without guiding, and becomes how much is left after paying, which you can decide on.",
    "isabella": "Architecture sells through the eye, so the project shows up big and early, with no text box competing with the photo. From portfolio to contact is a short walk: someone who likes a project wants to talk to whoever made it.",
    "locarmais-site": "Digital rent guarantee has three audiences that want opposite things: the tenant wants to rent without a deposit, the landlord wants a guarantee and the agency wants to close fast. The site tells the same story three times, each one through the reader's own problem.",
    "signamais": "Electronic signatures are sold by people who understand the law to people who do not, and the product usually comes out looking like the contract instead of like the person signing it. Here the sequence is the obvious one: upload the document, choose who signs, follow who already has. The legal rigour is all still there, just under the hood, which is where it does its job.",
    "rodape": "A book club is a fragile arrangement of a book, a date and some people, and it almost always dies in the group chat, where the choice of the next title gets lost between memes. The app holds the three in one place, with the current read always in sight and per-chapter comments behind a spoiler wall. Designed in Figma, still in closed testing.",
  ...
  PROJECTS.forEach((p) => { if (PJ_SOBRE_EN[p.id]) p.sobre = PJ_SOBRE_EN[p.id]; });
